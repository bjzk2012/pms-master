layui.use(['treeSelect', 'jquery', 'ztree', 'form', 'admin', 'ax', 'numinput'], function () {
    ...
    //获取文章目录信息
    var ajax = new $ax(Feng.ctxPath + "/catalogue/detail/" + Feng.getUrlParam("catalogueId"));
    ajax.type = "get";
    var result = ajax.start();
    form.val('catalogueForm', result);

    // 渲染父级文章目录信息
    treeSelect.render({
        elem: '#pName',
        data: Feng.ctxPath + "/catalogue/treeSelect",
        type: 'get',
        placeholder: '请选择上级文章目录',
        search: true,
        click: function(d){
    ...
    form.on('submit(btnSubmit)', function (data) {
        var ajax = new $ax(Feng.ctxPath + "/catalogue/edit", function (data) {
            Feng.success("修改成功！");
